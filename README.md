...
**Scoring is done by an LLM** (Gemini Flash via OpenRouter) reading a structured Markdown description of the occupation. The description is generated from COPS data and includes: NOC code, TEER level, major category, employment figures, labour market conditions, and projected job openings.
...
Current weighted average (job-weighted, 43 scored occupations): **5.4 / 10**.

---

## Pipeline

Five scripts take you from raw government data to the website:
...
| `site/index.html` | Self-contained frontend (treemap + scatter view) |
...
| `score.py` | Step 4 — LLM scoring (OpenRouter) |
...
Requires an [OpenRouter](https://openrouter.ai) API key for the scoring step:
...
# 4. Score AI exposure for each occupation  (needs OpenRouter key)
...
Steps 1–3 and 5 are deterministic and fast (seconds). Step 4 is the only step that calls an external API and costs money (~$0.50 for all 516 occupations with Gemini Flash).
...
- **AI scoring:** [Google Gemini Flash](https://deepmind.google/technologies/gemini/) via [OpenRouter](https://openrouter.ai)
